...
if [ -d "templates/go-gin-bearer-auth-postgres-monolithic-template" ]; then
        rm -rf "templates/go-gin-bearer-auth-postgres-monolithic-template"
fi

cd $TEMPLATE_PATH/templates
git clone https://github.com/golanguzb70/go-gin-bearer-auth-postgres-monolithic-template.git
cd go-gin-bearer-auth-postgres-monolithic-template
rm -rf .git


old_module="github.com/golanguzb70/go-gin-bearer-auth-postgres-monolithic-template"
...
process_files "$TEMPLATE_PATH/templates/go-gin-bearer-auth-postgres-monolithic-template"
...
process_files_to_edit() {
    ...
    for file in "${files[@]}"; do
        if [ -d "$file" ]; then
            if [[ "$file" != "$TEMPLATE_PATH/templates/go-gin-bearer-auth-postgres-monolithic-template/scripts" ]]; then
                echo $file
...
process_files_to_edit "$TEMPLATE_PATH/templates/go-gin-bearer-auth-postgres-monolithic-template"

# Copy handler level.
cp -r $TEMPLATE_PATH/templates/go-gin-bearer-auth-postgres-monolithic-template/api/handlers/v1/$crudname.go $current_path/api/handlers/v1/
# Copy database Level.
cp -r $TEMPLATE_PATH/templates/go-gin-bearer-auth-postgres-monolithic-template/storage/postgres/$crudname.go $current_path/storage/postgres/

# Copy models.
cp -r $TEMPLATE_PATH/templates/go-gin-bearer-auth-postgres-monolithic-template/models/$crudname.go $current_path/models
